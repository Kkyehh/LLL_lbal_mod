...
func add_conditional_effects(symbol, adjacent):
    for i in adjacent:
        ...
        symbol.add_effect_for_symbol(i, effect().if_final_value_less_than(0).set_target(symbol).change_value_bonus(i.non_prev_final_value * (-1 * values[1])).animate("shake", 0, [symbol, i]))